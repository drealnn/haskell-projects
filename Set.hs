...
myFunc :: [a] -> [[a]]
myFunc [] = [[]]
myFunc (x:xs) = (map (x:) $ myFunc(xs)) ++ (myFunc xs)


diff :: (Ord a) => Set a -> Set a -> Set a
diff (Set []) _ = Set []
diff (Set (x:xs)) (Set b) 
 | (memSet (Set b) x) = diff (Set xs) (Set b)
 | otherwise = Set ( x: (toList (diff (Set xs) (Set b))) )
	

symDiff :: (Ord a) => Set a -> Set a -> Set a
symDiff (Set a) (Set b) = diff (union (Set a) (Set b)) (inter (Set a) (Set b)) -- Take the difference between the union and the intersection

powerSet :: (Ord a) => Set a -> Set (Set a)
powerSet (Set []) = Set (Set []:[]) 
powerSet (Set (a)) = makeSet (map Set (myFunc $ toList (Set a))) -- implement it
